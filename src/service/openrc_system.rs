...
const INIT_PATH: &str = "/etc/init.d/aproman";

pub fn install() -> Result<()> {
    if !service::is_root() {
        bail!("OpenRC system install must be run as root");
    }
    let script = service::render_template(service_files::OPENRC_SYSTEM, "/usr/local/bin/aproman")?;
    ...
    run(&["rc-update", "add", "aproman", "default"])?;
    info!("Enabled aproman (system). Start with: rc-service aproman start");
    Ok(())
}
...
pub fn uninstall() -> Result<()> {
    ...
    let _ = run(&["rc-service", "aproman", "stop"]);
    let _ = run(&["rc-update", "del", "aproman", "default"]);
