...
pub fn install() -> Result<()> {
    ...
    let unit_path = unit_dir.join("aproman.service");
    let unit = service::render_template(service_files::SYSTEMD_UNIT, "%h/.local/bin/aproman")?;
    fs::write(&unit_path, unit).with_context(|| format!("writing {}", unit_path.display()))?;
    info!("Wrote {}", unit_path.display());
    run(&["systemctl", "--user", "daemon-reload"])?;
    run(&["systemctl", "--user", "enable", "aproman.service"])?;
    info!("Enabled aproman.service. Start with: systemctl --user start aproman.service");
    Ok(())
}
...
pub fn uninstall() -> Result<()> {
    ...
    let _ = run(&["systemctl", "--user", "disable", "--now", "aproman.service"]);
    let unit_path = user_unit_dir()?.join("aproman.service");
    if unit_path.exists() {
