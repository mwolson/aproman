...
pub fn install() -> Result<()> {
    ...
    let init_path = init_dir.join("aproman");
    let script = service::render_template(service_files::OPENRC_USER, "$HOME/.local/bin/aproman")?;
    ...
    run(&["rc-update", "--user", "add", "aproman", "default"])?;
    info!("Enabled aproman (user). Start with: rc-service --user aproman start");
    Ok(())
}
...
pub fn uninstall() -> Result<()> {
    ...
    let _ = run(&["rc-service", "--user", "aproman", "stop"]);
    let _ = run(&["rc-update", "--user", "del", "aproman", "default"]);
    let init_path = user_init_dir()?.join("aproman");
    if init_path.exists() {
